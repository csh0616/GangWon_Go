...
//
// 실행: `node ldong_lookup.js` (MEDICAL_TOURISM_SERVICE_KEY 필요)
// 라운드3(2026-09-10)에 이미 1회 실행해 medical_client.js의 REGION_TO_LDONG을 채웠다 —
// 재확인/재실행용으로 남겨둠. `langDivCd`가 필수 파라미터(실측 확인 — 없으면
// NO_MANDATORY_REQUEST_PARAMETERS_ERROR1).
require('dotenv').config();
const { callMedicalApi } = require('./medical_client');

async function main() {
  console.log('1단계: 시도 목록 조회 (lDongListYn=N) — 강원도 code 확인');
  const provinces = await callMedicalApi('ldongCode', { lDongListYn: 'N', langDivCd: 'ENG' });
  ...
  const districts = await callMedicalApi('ldongCode', { lDongListYn: 'Y', lDongRegnCd: gangwon.code, langDivCd: 'ENG' });
