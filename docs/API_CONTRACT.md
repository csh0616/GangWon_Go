...
2. `free_text`가 있으면 LLM 호출 — **프롬프트 지시문이 아니라 구조화 출력(스키마 강제)으로 형식을 보장한다**
   ...
   **예시 응답**
   ...
   > **가중치가 전부 0일 때 무엇을 고를 것인가 (신규 확정 — 2주차, 배포본 점검에서 발견)**
   >
   > 이 문서는 "가중치가 전부 0일 수 있다"고만 적어두고 **그럴 때 어떤 코스가 나와야 하는지는
   > 정하지 않았다.** 그 결과 배포본에서 자유 텍스트를 비우고 평창 3일 코스를 만들었더니
   > 스탑 12개 중 10개가 식당이었고, 순서가 **청춘카페 → 청춘보리밥 → 청산회관 → 초원식당 →
   > 청옥산 → 초원갈비 → 청성애원 → 촌닭 → 친정엄마 → 충주집 → 카누** 로 **가나다순**이었다.
   >
   > 원인은 동점 처리다. 모든 가중치가 0이면 `scorePoi`가 전 POI에 0점을 주고,
   > `.sort((a, b) => b.__score - a.__score)`는 동점일 때 입력 순서를 보존하므로 DB 조회 순서
   > (= TourAPI가 돌려준 가나다순)가 그대로 코스가 된다.
   >
   > **이것은 심사에서 가장 먼저 보이는 화면이다.** PRD 2.1절과 심사 동선 2단계(11.5절)가
   > "자유 텍스트는 비워도 진행됨"을 명시하고 화면에도 "비워도 괜찮아요"라고 적혀 있으므로,
   > **텍스트를 비우고 넘어가는 것은 우리가 권장한 경로다.** 그 경로에서 가나다순 식당 열 곳이
   > 나오면 나머지 기능이 전부 동작해도 인상이 거기서 결정된다.
   >
   > **확정 규칙 — 아래 셋을 모두 적용한다.**
   >
   > 1. **동점 타이브레이크를 알파벳과 무관한 결정적 기준으로 바꾼다.** `poi_id` 해시 등
   >    입력이 같으면 항상 같은 결과가 나오되 이름 순서와 상관없는 값을 쓴다. 난수는 쓰지 않는다
   >    — 심사위원이 새로고침할 때마다 코스가 바뀌면 신뢰를 잃는다.
   > 2. **하루 카테고리 다양성 제약**: 가중치와 무관하게 **하루 스탑의 과반이 같은 카테고리가
   >    되지 않게** 한다. 후보가 부족해 제약을 만족할 수 없으면 제약을 완화하되, 완화했다는
   >    사실을 로그로 남긴다(에러 아님 — 코스 생성은 계속되어야 한다).
   > 3. **동일 좌표 POI는 하나만 남긴다.** TourAPI가 같은 주소의 여러 상호에 동일 좌표를 주는
   >    경우가 있어(실측: 청춘카페&떡방 / 청춘보리밥 진부점이 소수점 10자리까지 동일) 지도 마커가
   >    완전히 겹치고, 카카오모빌리티가 출발지=도착지 경로를 계산하지 못해
   >    `[travel] 이동 정보 계산 실패` 로그와 함께 `travel_from_prev`가 `null`이 된다.
   >
   > 가중치가 **일부라도 0이 아닌 경우**에는 기존 스코어링이 그대로 우선한다 — 위 1·2는
   > 동점 구간에만 개입하고, 3은 항상 적용한다.
...
4. ~~LLM 내레이션 + 스탑 한 줄 설명 생성~~ → **이 단계는 `/generate`에서 빠졌다 (수정 — 2주차,
   실측 후 분리).** 아래 `POST /api/itineraries/narrate`로 옮겼으며, `/generate` 응답의
   `narration`/`region_reason`/모든 `blurb`는 **항상 `null`로 반환된다.** 스키마는 그대로 두므로
   프론트의 null 처리 경로가 그대로 쓰인다.

   > **왜 분리했나 (실측 근거)**: 3일/9스탑 코스 기준 `/generate` 총 응답시간이 **15.2초**였고,
   > 병렬화를 최대로 적용한 뒤에도 **6~9.6초**로 3초 예산을 못 지켰다. 내역을 보면 병목이
   > 한 곳이다 — 가중치 추출 1.2초 + pois 조회 0.15초 + 코스 생성 0.004초 + 카카오모빌리티
   > 0.4초(병렬화 후) = **약 1.8초**인데, narration/blurb 생성이 **5~8초**를 더한다.
   > 즉 **코스 뼈대 자체는 이미 3초 안에 나온다.** 설명을 같은 응답에 묶어두는 한 예산을
   > 지킬 방법이 없고, LLM 청크당 2~3초는 병렬화로도 줄지 않는 하한선이다(2스탑 청크 5개
   > 병렬 ≈3초 — 더 쪼개도 이득이 없는 지점 확인).
   >
   > 분리해도 화면이 깨지지 않는다: 프론트는 이미 `blurb`/`narration`이 `null`이면 그 줄을
   > 아예 그리지 않도록 구현돼 있다(§1 `blurb` 항목, QA T-019). 코스가 먼저 뜨고 설명이
   > 뒤이어 채워지는 형태가 된다.

   참고로 아래 스키마의 "한 번의 호출" 표현은 **"직렬 호출로 지연이 배가되는 것을 막는다"는
   의도**였는데, 실측 결과 그 한 번의 호출 자체가 출력 토큰(≈1231, ≈107 tokens/sec) 때문에
   10~12초가 걸렸다. 따라서 narrate 엔드포인트 안에서는 **작은 호출 여러 개를 `Promise.all`로
   동시에** 발사한다(narration+region_reason 1개 + blurb 3스탑씩 묶은 청크 N개). 지연이
   "전체 출력 토큰 합"이 아니라 "가장 느린 한 묶음"만큼만 걸린다.

   > **`max_tokens` 절단 주의 (2주차 실측에서 발견)**: tool_use 인자 JSON이 `max_tokens`에
   > 걸려 잘리면 Anthropic SDK가 예외를 던지지 않고 `input: {}`(빈 객체)를 그대로 준다 —
   > narration이 아무 경고 로그 없이 조용히 전부 `null`이 된다. `stop_reason === 'max_tokens'`
   > 일 때 명시적으로 실패 처리할 것.

   **출력 스키마 (narrate 엔드포인트에서 사용)**:
   ```json
   {
     "type": "object",
     "properties": {
...
> **`stops[].category`는 7개 마스터 키만 내보낸다 — TourAPI 원본 코드가 아니다 (2주차 확정, 실배포 장애로 발견)**
>
> 배포본 실연동 첫 시도에서 **코스 결과 화면이 통째로 죽었다.** 원인은 응답의
> `stops[].category`에 `"A01010900"`(TourAPI cat3 원본 코드)이 실려 나간 것이었다. 프론트의
> 메시지 카탈로그(`app/messages/*.json`)에는 7개 마스터 키만 있으므로 `categories.A01010900`
> 조회가 실패 → next-intl이 예외를 던짐 → React 렌더 트리 전체 붕괴 → `This page couldn't load`.
>
> **DB에는 두 값이 따로 들어 있다.** `scripts/sync/sync_pois.js`가
> `category: raw.cat3 || raw.cat2 || raw.cat1`(원본 코드)와 `tags: [매핑된 7개 키 중 하나]`를
> 각각 저장한다. 매핑 규칙(`scripts/sync/category_mapping.js`)은 이미 올바르게 동작하고 있으며,
> **결과를 `tags`에만 넣고 응답에는 `pois.category`를 그대로 실은 것이 유일한 문제였다.**
>
> - **서버는 `stops[].category`에 `tags[0]`(7개 마스터 키)을 내보낸다.** `pois.category`는
>   TourAPI 원본 보존용 컬럼으로 남긴다 — 재동기화 없이 서버 응답 매핑만 고치는 쪽을 택한다
>   (9/21 마감까지 3개 시군 전 POI 재동기화를 다시 돌릴 여유가 없다).
> - 이 규칙은 `/generate`, `/regenerate-stop`, `/itineraries/:id`, `/alerts` 등 **스탑을 내보내는
>   모든 응답에 동일하게 적용된다.** 한 곳만 고치면 다른 화면에서 같은 사고가 재발한다.
> - `tags`가 비어 있는 POI는 애초에 동기화 대상에서 제외되므로(§ `category_mapping.js`가 `null`
>   반환 시 스킵) 정상 경로에서 `category`가 `null`이 될 일은 없다. 그래도 방어 규칙은 아래를 따른다.
>
> **프론트 방어 규칙 (필수)**: 7개 키가 아닌 값이 오면 **라벨만 비우고 스탑은 그대로 렌더링한다.**
> 카테고리 하나를 모른다고 화면 전체가 죽어서는 안 된다 — §4 케어 시설의 `category` 규칙과 같은
> 원칙이다(PRD 6장 "실패한 조각만 조용히 대체, 핵심 경험은 막지 않음"). 심사위원이 배포 URL에
> 직접 접속하는 방식이므로(PRD 11.5절) **렌더 중 예외 한 번이 심사 동선 전체를 끊는다.**
      ...
      "days": [
        ...
        },
        {
          ...
          "travel_from_prev_day": {
            "from_region_code": "pyeongchang",
            "to_region_code": "hongcheon",
            "mode": "car",
            "minutes": 70
          },
          "stops": []
...
> **어디에 표시할지 (확정 — UI 아트보드 검수에서 규칙 부재 발견)**: **교체 후보를 고르는 맥락에서만
> 배지로 표시한다** — 매니징 제안 모달과 "이 장소 바꾸기" 후보 목록. 그 두 곳에서만 실내/야외가
> 선택의 근거이기 때문이다. **코스 결과 화면의 스탑 카드에는 배지를 붙이지 않는다.**
> 검수 시점의 아트보드는 결과 화면에서 실내 여부를 `blurb` 문장 안에 녹여 표현하고 있었는데
> (예: "축제장 바로 옆, 비 와도 괜찮은 실내"), `blurb`는 LLM이 만든 문장이라 **실내인데도 그 단어가
> 안 들어간 장소는 아무 표시가 없게 된다.** 데이터 필드와 생성 문장이 같은 정보를 두 방식으로
> 나타내면 화면이 일관되지 않으므로, 결과 화면은 `blurb`에 맡기고 배지는 쓰지 않는 쪽으로 통일한다.
...
**`days[].travel_from_prev_day` — 전날 마지막 스탑에서 이 날 첫 스탑까지의 이동**

`{from_region_code, to_region_code, mode, minutes}` 또는 `null`.

**왜 필요한가 (UI 아트보드 검수에서 발견)**: 다중 시군 코스에서 **날이 바뀌면서 시군도 바뀌는 구간의
이동이 화면 어디에도 없었다.** 하루 안의 스탑 사이는 `travel_from_prev`로 "차로 32분"을 보여주는데,
홍천에서 자고 평창으로 넘어가는 구간은 그날 첫 스탑이라 `travel_from_prev`가 `null`이라 아무것도
표시되지 않았다. 사용자에게 이건 **숙소를 옮겨야 하는지를 결정하는 정보**이므로 빠지면 다중 시군
기능이 반쪽이 된다.

- **`day: 1`에서는 항상 `null`** (전날이 없음).
- 전날에 스탑이 하나도 없으면(`stops: []`) **그보다 더 이전의, 스탑이 있는 마지막 날**을 기준으로
  계산한다. 빈 날 때문에 이동 정보가 끊기지 않게 하기 위함이다. 그런 날조차 없으면 `null`.
- `from_region_code`와 `to_region_code`가 **같아도 값을 내려보낸다** — 같은 시군 안에서도 날이
  바뀌며 이동이 생기기 때문이다. 프론트는 두 값이 다를 때만 "홍천 → 평창"처럼 시군 이름을 함께
  보여주고, 같으면 이동시간만 표시한다.
- `mode`/`minutes`는 `travel_from_prev`와 같은 규칙이며, 카카오모빌리티 호출 실패 시 **필드 전체를
  `null`** 로 둔다(추정값을 지어내지 않는다).
...
4. `days[i].region_code`는 **항상 단일 시군**이며 요청한 `region_codes` 안의 값이다 (신규 —
   ...
   여행 일수` 검증이 이를 보장)
5. `days[0].travel_from_prev_day`는 **항상 `null`** 이며, 그 외의 날은 전날(또는 스탑이 있는 가장
   가까운 이전 날)로부터의 이동을 담거나 계산 실패 시 `null`이다 (신규)
...
**수용 기준**: PRD 2.1절 — **3초 이내 응답**. 2주차 실측 기준 뼈대 생성은 약 1.8초
(가중치 추출 1.2 + pois 조회 0.15 + 코스 생성 0.004 + 카카오모빌리티 병렬 0.4)이며, 설명 생성은
`narrate`로 분리되어 이 예산에 포함되지 않는다. 이 시점엔 `itineraries` row를 만들지 않는다
(게스트 상태, 클라이언트 메모리 보관).

---

### `POST /api/itineraries/narrate` (설명 채우기 — 신규, 2주차 확정)

`/generate`가 뼈대만 빠르게 돌려준 뒤, 코스 요약과 스탑 설명을 채우기 위해 **프론트가 이어서
호출하는** 엔드포인트. 게스트도 호출 가능(인증 불필요) — 저장 전 코스에도 설명이 필요하다.
  ...
  "days": [
    {
      ...
      "region_code": "pyeongchang",
      "stops": [
        { "poi_id": "poi_123", "name_ko": "월정사 전나무숲길", "category": "nature_hiking" }
      ]
    }
  ]
}
...
- **`/generate` 응답을 그대로 되돌려보내지 않는다** — 프롬프트에 필요한 값(장소명·카테고리·
  시군)만 담는다. 좌표·이동시간·`is_indoor`는 보내지 않는다.
- 서버는 **영업시간·가격·연락처·휴무일을 프롬프트에 넣지도, 생성하게 하지도 않는다**(§1 `blurb`).
...
```json
{
  "data": {
    "narration": { "ko": "...", "en": "...", "zh": "..." },
    "region_reason": { "ko": "...", "en": "...", "zh": "..." },
    "blurbs": [
      { "poi_id": "poi_123", "ko": "천년 전나무가 늘어선 1km 흙길", "en": "...", "zh": "..." }
    ]
  },
  "error": null
}
...
- `blurbs[]`의 `poi_id`가 요청에 없던 값이면 **서버가 버린다**(LLM이 지어낸 장소 차단).
- `selected_regions.auto`가 `false`면 `region_reason`은 **`null`** — 사용자가 고른 것을
  시스템이 골라준 것처럼 말하지 않는다.
- **실패해도 `/generate` 결과에는 영향이 없다.** LLM 실패 시 세 값을 모두 `null`로 담아
  200으로 반환한다(에러 코드를 쓰지 않는다 — 프론트가 "설명 없는 코스"를 그대로 유지하면 되고,
  실패를 사용자에게 알릴 이유가 없다).
- **수용 기준**: 이 호출은 3초 예산의 대상이 아니다. 다만 10초를 넘기면 프론트가 포기하고
  설명 없이 남긴다(아래).

**프론트 처리 (확정)**

1. `/generate` 응답이 오면 **즉시 코스를 그린다.** 설명 줄은 `null`이므로 안 그려진다.
2. 곧바로 `narrate`를 호출하고, 응답이 오면 `poi_id`로 매칭해 채워 넣는다.
   채워질 때 레이아웃이 크게 흔들리지 않도록 부드럽게 붙인다.
3. **로딩 스피너를 두 번 보여주지 않는다** — 코스는 이미 떠 있고 설명만 나중에 붙는 것이므로,
   설명 자리에 스켈레톤이나 "생성 중" 문구를 두지 않는다. 조용히 채워지면 된다.
4. 10초 타임아웃 또는 실패 시 재시도하지 않고 설명 없이 둔다. 사용자에게 에러를 띄우지 않는다.
5. 코스를 저장(§2)할 때는 **채워진 설명이 포함된 `itinerary_json`** 을 보낸다. 아직 안 채워졌으면
   `null`인 채로 저장되며, 그건 정상이다.
...
- `category`는 다국어 키로 처리한다 (`emergency_room` / `health_center` / `health_subcenter` /
  `hospital` / **`clinic`**) — UI 라벨은 프론트가 언어별로 렌더링.

  > **`clinic` 추가 (2주차, 병·의원 데이터셋 도입에 따라)**: 전국 병·의원 찾기 서비스의
  > `dutyDivNam`을 매핑한 결과 **의원 / 한의원 / 치과의원**이 새 값으로 필요해졌다.
  > 실제 분포상 이 값이 가장 많으므로(인제 29건 중 의원계 17건) 프론트는 반드시 라벨을
  > 준비해야 한다. `health_subcenter`는 이 데이터셋이 보건소/보건지소를 세분하지 않고
  > 전부 "보건소"로만 주므로 **당분간 값이 들어오지 않는다**(키는 유지 — 소스가 늘면 다시 쓴다).
  >
  > `care_facilities.category`는 DB enum이 아니라 plain text 컬럼이라 마이그레이션이
  > 필요 없다. 다만 **프론트가 모르는 값을 받으면 라벨이 비므로**, 알 수 없는 `category`는
  > 시설 종류 줄을 생략하고 이름·거리만 표시한다(빈 라벨을 그리지 않는다).

- **`name_en`/`name_zh` 문자 검증 (확정 — 2주차, 번역 품질 이슈 발견)**: 동기화 시 LLM이
  지명을 번역하는데, 결과에 **키릴 문자 등 엉뚱한 문자가 섞이는 사례**가 실측으로 확인됐다
  (예: "약손한의원" → `name_en`에 키릴 문자 혼입). 시설 종류 접미사는 고정 사전이라 안전하지만
  지명 부분은 검증이 없었다. **동기화 시점에 문자 클래스를 검사해 통과 못 하면 그 언어를
  `null`로 떨어뜨린다** — 화면에는 한국어 원문만 나오고, 이상한 문자가 사용자에게 보이지 않는다.
  - `name_en`: ASCII 문자·숫자·공백·하이픈·아포스트로피·마침표·괄호만 허용. 그 외 문자가
    하나라도 있으면 `null`.
  - `name_zh`: CJK 한자 + ASCII 숫자/공백/괄호만 허용. **한글·키릴이 하나라도 있으면 `null`.**
  - 같은 규칙을 `pois.name_en`/`name_zh`에도 적용한다. 사람이 652건을 육안으로 검수하는 것은
    현실적이지 않으므로 기계적으로 막는다.
